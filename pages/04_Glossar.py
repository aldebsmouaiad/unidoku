...
def _do_return(aid: str, ret: str, payload: dict) -> None:
    """
    Zurück-Navigation inkl. Restore relevanter Erhebung-State + Cleanup.
    """
    if ret == "Erhebung":
...
def main() -> None:
    ...
    # AID + Restore wirklich vor ALLEN Widgets
    ...
    # Tokens (wie in deinen fertigen Seiten)
...
<style>
  ...
  /* "Card"-Container via Anchor+Sibling (funktioniert zuverlässig mit Streamlit Widgets) */
  ...
  /* Expander wie Cards (professioneller Look) */
  ...
  /* =========================================
     NAV-BUTTONS: Secondary konsistent wie auf deinen fertigen Seiten
     ========================================= */
...
<div class="rgm-hero">
  ...
  <p class="rgm-lead">
    ...
    # Top-Navigation (nur wenn Rücksprung vorhanden)
    if ret:
        st.markdown('<div class="rgm-subtle-divider"></div>', unsafe_allow_html=True)
        c1, c2 = st.columns([1, 3])
        with c1:
            if st.button("Zurück", key="glossar_back_btn_top", use_container_width=True):
                _do_return(aid=aid, ret=ret, payload=payload)
        with c2:
            st.markdown(
                "<div class='rgm-muted' style='margin-top:6px'>"
    ...
    # Bottom-Navigation (optional hilfreich nach langem Scroll)
    if ret:
        st.markdown("---")
